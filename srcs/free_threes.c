...
float		free_three2(int x, int y, int xd, int yd)
{
	...
	if (!(x - xd > -1 && x - xd < 19 && y - yd > -1 && y - yd < 19) ||
			g_e->gomoku.map[0][y - yd][x - xd] != v.p)
		return (0);
...
float		free_three(int x, int y, int xd, int yd)
{
	...
	v.c = -1;
	bzero(a, 7);
	v.o = 2 - (g_e->gomoku.player_turn == 2);
	if (!(x - xd > -1 && x - xd < 19 && y - yd > -1 && y - yd < 19) ||
			g_e->gomoku.map[0][y - yd][x - xd])
		return (free_three2(x, y, xd, yd));
	a[++v.c] = g_e->gomoku.map[0][y][x] + 48 - (N_MAP[y][x] == 2);
...
void		double_free_threes(void)
{
	INIT_10;
	y = -1;
	p = g_e->gomoku.player_turn;
	while (++y < 19 && (x = -1))
	{
		while (++x < 19)
		{
			c = 0.0;
			if (!g_e->gomoku.map[0][y][x])
			{
				g_e->gomoku.map[0][y][x] = p;
				...
				c += free_three(x, y, -1, 0);
				c >= 2 && (g_e->gomoku.map[3][y][x] = c);
				g_e->gomoku.map[0][y][x] = 0;
			}
		}
	}
}
