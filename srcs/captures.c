...
void    possible_captures(int n, int o, int p)
{
    ...
    while (++y < 18 && (x = -1))
    {
        while (++x < 18)
        {
            ...
            {
                x + 3 < 19 && M_POS_20 && M_POS_21 && M_POS_22 &&
                    (e->gomoku.map[n][y][x] += 1);
                x - 3 >= 0 && M_POS_23 && M_POS_24 && M_POS_25 &&
                    (e->gomoku.map[n][y][x] += 1);
                y + 3 < 19 && M_POS_26 && M_POS_27 && M_POS_28 &&
                    (e->gomoku.map[n][y][x] += 1);
                y - 3 >= 0 && M_POS_29 && M_POS_30 && M_POS_31 &&
                    (e->gomoku.map[n][y][x] += 1);
                x + 3 < 19 && M_POS_32 && M_POS_33 && M_POS_35 &&
                    (e->gomoku.map[n][y][x] += 1);
